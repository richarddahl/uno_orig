...
def create_audit_table(table_name: str):
    ...
    return f"""
        ...
        REVOKE ALL ON audit.{table_name}
        FROM public;
        COMMENT ON TABLE audit.{table_name} IS 'History of auditable actions on audited tables, from audit.if_{table_name}_modified_func()';
        COMMENT ON COLUMN audit.{table_name}.event_id IS 'Unique identifier for each auditable event';
        COMMENT ON COLUMN audit.{table_name}.schema_name IS 'Database schema audited table for this event is in';
        COMMENT ON COLUMN audit.{table_name}.relid IS 'Table OID. Changes with drop/create. Get with ''tablename''::regclass';
        COMMENT ON COLUMN audit.{table_name}.session_user_name IS 'Login / session user whose statement caused the audited event';
        COMMENT ON COLUMN audit.{table_name}.app_user IS 'The application user that initiated the event';
        COMMENT ON COLUMN audit.{table_name}.action_tstamp_tx IS 'Transaction start timestamp for tx in which audited event occurred';
        COMMENT ON COLUMN audit.{table_name}.action_tstamp_stm IS 'Statement start timestamp for tx in which audited event occurred';
        COMMENT ON COLUMN audit.{table_name}.action_tstamp_clk IS 'Wall clock time at which audited event''s trigger call occurred';
        COMMENT ON COLUMN audit.{table_name}.transaction_id IS 'Identifier of transaction that made the change. May wrap, but unique paired with action_tstamp_tx.';
        COMMENT ON COLUMN audit.{table_name}.client_addr IS 'IP address of client that issued query. Null for unix domain socket.';
        COMMENT ON COLUMN audit.{table_name}.client_port IS 'Remote peer IP port address of client that issued query. Undefined for unix socket.';
        COMMENT ON COLUMN audit.{table_name}.client_query IS 'Top-level query that caused this auditable event. May be more than one statement.';
        COMMENT ON COLUMN audit.{table_name}.application_name IS 'Application name set when this audit event occurred. Can be changed in-session by client.';
        COMMENT ON COLUMN audit.{table_name}.action IS 'Action type; I = insert, D = delete, U = update, T = truncate';
        COMMENT ON COLUMN audit.{table_name}.row_data IS 'Record value. Null for statement-level trigger. For INSERT this is the new tuple. For DELETE and UPDATE it is the old tuple.';
        COMMENT ON COLUMN audit.{table_name}.changed_fields IS 'New values of fields changed by UPDATE. Null except for row-level UPDATE events.';
        COMMENT ON COLUMN audit.{table_name}.statement_only IS '''t'' if audit event is from an FOR EACH STATEMENT trigger, ''f'' for FOR EACH ROW';
        CREATE INDEX {table_name}_relid_idx ON audit.{table_name}(relid);
        CREATE INDEX {table_name}_action_tstamp_tx_stm_idx ON audit.{table_name}(action_tstamp_stm);
        CREATE INDEX {table_name}_action_idx ON audit.{table_name}(action);


        -- Function to create the trigger function
        CREATE OR REPLACE FUNCTION audit.if_{table_name}_modified_func() RETURNS TRIGGER AS $body$
        DECLARE
            audit_row audit.{table_name};
            include_values boolean;
            log_diffs boolean;
            h_old jsonb;
            h_new jsonb;
            excluded_cols text [] = ARRAY []::text [];
            BEGIN IF TG_WHEN <> 'AFTER' THEN RAISE EXCEPTION 'audit.if_{table_name}_modified_func() may only run as an AFTER trigger';
        END IF;
        audit_row = ROW(
            nextval('audit.{table_name}_event_id_seq'),
            -- event_id
        ...
        ELSE RAISE EXCEPTION '[audit.if_{table_name}_modified_func] - Trigger func added as trigger for unhandled case: %, %',
        TG_OP,
        TG_LEVEL;
        RETURN NULL;
        END IF;
        ...
        SET search_path = pg_catalog,
            public;
        COMMENT ON FUNCTION audit.if_{table_name}_modified_func() IS $body$ Track changes to a table at the statement
        and /
        ...
        _q_txt = 'CREATE TRIGGER audit_trigger_row AFTER ' || CASE
            ...
            ELSE ''
        END || 'UPDATE OR DELETE ON ' || target_table || ' FOR EACH ROW EXECUTE PROCEDURE audit.if_{table_name}_modified_func(' || quote_literal(audit_query_text) || _ignored_cols_snip || ');';
        RAISE NOTICE '%',
        _q_txt;
        EXECUTE _q_txt;
        ...
        ELSE
        END IF;
        _q_txt = 'CREATE TRIGGER audit_trigger_stm AFTER ' || stm_targets || ' ON ' || target_table || ' FOR EACH STATEMENT EXECUTE PROCEDURE audit.if_{table_name}_modified_func(' || quote_literal(audit_query_text) || ');';
        RAISE NOTICE '%',
        _q_txt;
        EXECUTE _q_txt;
        END;
...
def enable_auditing(table_name: str):
    ...
    return f"""
        ...
        CREATE TRIGGER audit_trigger_{_table_name}_row
            AFTER INSERT OR DELETE OR UPDATE 
            ON {table_name}
            FOR EACH ROW
            EXECUTE FUNCTION audit.if_{_table_name}_modified_func('true');
    """
